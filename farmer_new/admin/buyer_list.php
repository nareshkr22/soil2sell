...
<body>
	...
	<?php
        require('data_connection.php');
            $smt = $conn->prepare('SELECT * FROM `db_user_registration` WHERE   ur_grpid=3 and ur_tag=1');
            $smt->execute();
            $data = $smt->fetchAll()
        ?>
	<?php
  ...
  <table class="table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?php 
              echo "<a href='edit_list.php?eid=".$row['ur_id']."&edit=edit' rel='tooltip'onClick='(".$row['ur_id'].")'><i class='fa fa-edit'></i></a>"
